...
export default function GlobalNotFound() {
	return (
		<html lang="en" suppressHydrationWarning>
			<body
				className={`${manrope.variable} antialiased min-h-screen bg-background text-foreground flex flex-col items-center justify-center overflow-hidden`}
			>
				<ThemeProvider
					attribute="class"
					defaultTheme="system"
							...
							<div className="absolute bottom-4 left-4 right-4 bg-black/80 p-3 rounded-lg text-sm font-mono text-green-400 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none text-left">
								<div className="flex flex-col gap-1">
									...
									<span>
										...
										<span className="text-red-400 animate-pulse font-extrabold">
											CRITICAL
										</span>
									</span>
								</div>
						...
						<div className="flex flex-col items-center md:items-start text-center md:text-left space-y-6 md:space-y-8 order-1 md:order-2 z-10">
							...
							<div className="space-y-4 max-w-lg">
								...
								<p className="text-muted-foreground text-lg md:text-xl leading-relaxed">
									We explicitly told the AI to{" "}
									<span className="text-foreground font-semibold italic">
										&quot;stick to the context info only&quot;
									</span>
									, but it decided to dream up this page anyway.
								</p>
								<p className="text-sm text-muted-foreground/80 italic">
									(It probably thinks this page is real. It&apos;s not. Please
									forgive it.)
								</p>
							</div>
							...
							<Button
								asChild
								size="lg"
								className="rounded-full px-10 h-14 text-lg font-bold shadow-lg hover:shadow-xl transition-all hover:-translate-y-1 w-full md:w-auto"
							>
								<a href="/">Return to Reality</a>
							</Button>
						</div>
					</div>
